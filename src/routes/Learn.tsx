...
export default function Learn() {
  ...
  useEffect(() => { void Promise.all([recentAttempts(100), lessonProgresses(), loadAll()]).then(([a,p,q]) => { setAttempts(a); setProgress(p); setQuestions(q) }) }, [])
  const recommendations = useMemo(() => attempts && progress ? recommendLessons(attempts, progress) : [], [attempts, progress])
  if (!attempts || !progress || !questions) return <Spinner label="Loading lessons" />
  ...
  return <div className="mx-auto max-w-5xl space-y-7">
    ...
    <section><h2 className="font-semibold">Full syllabus</h2><p className="mt-1 text-sm text-muted">Every question is organised by its handbook section. Open a category to learn each fact in context.</p><div className="mt-3 space-y-4">{CHAPTERS.map((chapter) => { const sections = new Map<string, number>(); questions.filter((question) => question.chapter === chapter.id).forEach((question) => sections.set(question.section, (sections.get(question.section) ?? 0) + 1)); return <div key={chapter.id}><h3 className="text-sm font-medium">{chapter.short}</h3><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{[...sections.entries()].map(([section,count]) => <ButtonLink key={section} to={`/study/topics/${chapter.id}/${encodeURIComponent(section)}`} variant="secondary" className="justify-between text-left"><span className="truncate">{section}</span><span className="shrink-0 text-xs text-muted">{count}</span></ButtonLink>)}</div></div>})}</div></section>
